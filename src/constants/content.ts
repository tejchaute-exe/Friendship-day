export const TOTAL_STEPS = 8;

export const LOADING_MESSAGES = [
  'Folding a tiny paper plane...',
  'Collecting good vibes...',
  'Removing awkward silence...',
  'Finding the right words...',
  'Counting random topics...',
  'Checking the friendship radar...',
  'Brewing something warm...',
  'Untangling a few thoughts...',
  'Polishing tiny stars...',
  'Putting a small bow on this...',
];

export const IDENTITY_CHECKS = [
  'Human',
  'Curious',
  'Talks a lot after getting comfortable',
  'Makes random conversations fun',
  'Pretty awesome',
];

export const ACHIEVEMENTS = [
  {
    title: 'Pocket-Sized Legend',
    emoji: '🤏',
    note: 'Small in height. Massive in personality.',
  },
  {
    title: 'Professional Yapper',
    emoji: '💬',
    note: 'Could probably talk about almost anything.',
  },
  {
    title: 'Never-Gives-Up Award',
    emoji: '💪',
    note: 'Stubborn enough to call it determination.',
  },
  {
    title: 'Conversation Expert',
    emoji: '🌙',
    note: 'Time flies surprisingly fast.',
  },
  {
    title: 'Short Temper, Good Heart',
    emoji: '😤',
    note: 'Gets irritated quickly... but never stays that way for long.',
  },
];

export const CONFIDENTIAL_LINES = [
  'Sometimes...',
  "people become part of your day",
  'without you even realizing it.',
  '',
  'Not because of anything big...',
  'but because the little moments',
  'quietly add up.',
  '',
  'So...',
  'just wanted to say',
  'I appreciate those moments.',
  '',
  'Keep smiling...',
  'Good Girl. 🌼',
];
